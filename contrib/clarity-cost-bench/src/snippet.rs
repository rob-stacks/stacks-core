...
pub enum Snippet {
    ...
    SliceBuf,       SliceStr,       SliceList(ElemType),
    ReplaceAtBuf,   ReplaceAtStr,   ReplaceAtList(ElemType),
    AsMaxLenBuf,    AsMaxLenStr,    AsMaxLenList(ElemType),

    // ── higher-order (inner function kept, outer from suite) ─────────────
    /// `(op fn_name (list …))`
    MapList    { elem: ElemType, fn_name: &'static str },
    FilterList { elem: ElemType, fn_name: &'static str },
    /// `(op fn_name (list …) init)`
    FoldList   { elem: ElemType, fn_name: &'static str, init: &'static str },
...
impl Snippet {
    pub const fn variant(&self) -> &'static str {
        match self {
            ...
            // buffers
            Snippet::ApplyBuf | Snippet::TwoHalfBuf | Snippet::TwoBuf
            | Snippet::SliceBuf | Snippet::ReplaceAtBuf | Snippet::AsMaxLenBuf
            | Snippet::BuffToInt | Snippet::ToConsensusBuf | Snippet::FromConsensusBuf
            | Snippet::GetBitcoinTxOutput | Snippet::Ed25519Verify
            | Snippet::ElementAtBuf | Snippet::IndexOfBuf => "buffer",

            // strings
            Snippet::ApplyStr | Snippet::TwoHalfStr | Snippet::TwoStr
            | Snippet::SliceStr | Snippet::ReplaceAtStr | Snippet::AsMaxLenStr
            | Snippet::StringToIntAscii | Snippet::IndexOfStr
            ...
            Snippet::ApplyList(ElemType::Uint)     | Snippet::TwoHalfList(ElemType::Uint)
            | Snippet::TwoList(ElemType::Uint)     | Snippet::AppendList(ElemType::Uint)
            | Snippet::ElementAtList(ElemType::Uint) | Snippet::IndexOfList(ElemType::Uint)
            | Snippet::SliceList(ElemType::Uint)   | Snippet::ReplaceAtList(ElemType::Uint)
            | Snippet::AsMaxLenList(ElemType::Uint)
            | Snippet::MapList { elem: ElemType::Uint, .. }
            | Snippet::FilterList { elem: ElemType::Uint, .. }
            | Snippet::FoldList { elem: ElemType::Uint, .. } => "list-uint",

            Snippet::ApplyList(ElemType::Int)      | Snippet::TwoHalfList(ElemType::Int)
            | Snippet::TwoList(ElemType::Int)      | Snippet::AppendList(ElemType::Int)
            | Snippet::ElementAtList(ElemType::Int)| Snippet::IndexOfList(ElemType::Int)
            | Snippet::SliceList(ElemType::Int)    | Snippet::ReplaceAtList(ElemType::Int)
            | Snippet::AsMaxLenList(ElemType::Int)
            | Snippet::MapList { elem: ElemType::Int, .. }
            | Snippet::FilterList { elem: ElemType::Int, .. }
            | Snippet::FoldList { elem: ElemType::Int, .. } => "list-int",

            Snippet::ApplyList(ElemType::Bool)     | Snippet::TwoHalfList(ElemType::Bool)
            | Snippet::TwoList(ElemType::Bool)     | Snippet::AppendList(ElemType::Bool)
            | Snippet::ElementAtList(ElemType::Bool) | Snippet::IndexOfList(ElemType::Bool)
            | Snippet::SliceList(ElemType::Bool)   | Snippet::ReplaceAtList(ElemType::Bool)
            | Snippet::AsMaxLenList(ElemType::Bool)
            | Snippet::MapList { elem: ElemType::Bool, .. }
            | Snippet::FilterList { elem: ElemType::Bool, .. }
            | Snippet::FoldList { elem: ElemType::Bool, .. } => "list-bool",

            Snippet::ApplyList(ElemType::AsciiStr)   | Snippet::TwoHalfList(ElemType::AsciiStr)
            | Snippet::TwoList(ElemType::AsciiStr)   | Snippet::AppendList(ElemType::AsciiStr)
            | Snippet::ElementAtList(ElemType::AsciiStr) | Snippet::IndexOfList(ElemType::AsciiStr)
            | Snippet::SliceList(ElemType::AsciiStr) | Snippet::ReplaceAtList(ElemType::AsciiStr)
            | Snippet::AsMaxLenList(ElemType::AsciiStr)
            | Snippet::MapList { elem: ElemType::AsciiStr, .. }
            | Snippet::FilterList { elem: ElemType::AsciiStr, .. }
            | Snippet::FoldList { elem: ElemType::AsciiStr, .. } => "list-string-ascii",

            Snippet::ApplyList(ElemType::Buf)      | Snippet::TwoHalfList(ElemType::Buf)
            | Snippet::TwoList(ElemType::Buf)      | Snippet::AppendList(ElemType::Buf)
            | Snippet::ElementAtList(ElemType::Buf)| Snippet::IndexOfList(ElemType::Buf)
            | Snippet::SliceList(ElemType::Buf)    | Snippet::ReplaceAtList(ElemType::Buf)
            | Snippet::AsMaxLenList(ElemType::Buf)
            | Snippet::MapList { elem: ElemType::Buf, .. }
            | Snippet::FilterList { elem: ElemType::Buf, .. }
            | Snippet::FoldList { elem: ElemType::Buf, .. } => "list-buffer",

            Snippet::ApplyList(ElemType::OptUint)     | Snippet::TwoHalfList(ElemType::OptUint)
            | Snippet::TwoList(ElemType::OptUint)     | Snippet::AppendList(ElemType::OptUint)
            | Snippet::ElementAtList(ElemType::OptUint) | Snippet::IndexOfList(ElemType::OptUint)
            | Snippet::SliceList(ElemType::OptUint)   | Snippet::ReplaceAtList(ElemType::OptUint)
            | Snippet::AsMaxLenList(ElemType::OptUint)
            | Snippet::MapList { elem: ElemType::OptUint, .. }
            | Snippet::FilterList { elem: ElemType::OptUint, .. }
            | Snippet::FoldList { elem: ElemType::OptUint, .. } => "list-optional-uint",
    ...
    pub const fn n_unit(&self) -> &'static str {
        match self {
            ...
            | Snippet::BuffToInt | Snippet::ToConsensusBuf | Snippet::FromConsensusBuf
            | Snippet::GetBitcoinTxOutput | Snippet::Ed25519Verify
            | Snippet::ElementAtBuf | Snippet::IndexOfBuf => "buffer_bytes",

            Snippet::ApplyStr | Snippet::TwoHalfStr | Snippet::TwoStr
            | Snippet::SliceStr | Snippet::ReplaceAtStr | Snippet::AsMaxLenStr
            | Snippet::StringToIntAscii | Snippet::StringToIntUtf8
            | Snippet::ApplyUtf8 | Snippet::IndexOfStr
            | Snippet::ElementAtStr => "string_chars",

            // all list operations
            Snippet::ApplyList(_)   | Snippet::TwoHalfList(_) | Snippet::TwoList(_)
            | Snippet::AppendList(_)| Snippet::ElementAtList(_)| Snippet::IndexOfList(_)
            | Snippet::SliceList(_) | Snippet::ReplaceAtList(_)| Snippet::AsMaxLenList(_)
            | Snippet::MapList { .. }| Snippet::FilterList { .. }| Snippet::FoldList { .. }
            => "list_length",

            Snippet::VarOf(_) | Snippet::VarLeadUint | Snippet::VarLeadInt
            | Snippet::VarFalse => "arg_count",
    ...
    pub const fn default_sizes(&self) -> &'static [u64] {
        match self {
            ...
            | Snippet::GetBitcoinTxOutput | Snippet::Ed25519Verify
            | Snippet::ElementAtBuf | Snippet::IndexOfBuf
            | Snippet::ApplyStr | Snippet::TwoHalfStr | Snippet::TwoStr
            | Snippet::SliceStr | Snippet::ReplaceAtStr | Snippet::AsMaxLenStr
            | Snippet::IndexOfStr | Snippet::ElementAtStr | Snippet::ApplyUtf8
...
fn buf(n: u64) -> String  { format!("0x{}", "ab".repeat(n.max(1) as usize)) }
fn astr(n: u64) -> String { format!("\"{}\"", "a".repeat(n.max(1) as usize)) }
fn ustr(n: u64) -> String { format!("u\"{}\"", "a".repeat(n.max(1) as usize)) }
...
impl Snippet {
    pub fn generate(&self, op: &str, n: u64) -> String {
        match self {
            ...
            Snippet::TwoHalfBuf => { let h = n.max(2)/2; format!("({op} {} {})", buf(h), buf(h)) }
            Snippet::TwoHalfStr => { let h = n.max(2)/2; format!("({op} {} {})", astr(h), astr(h)) }
            ...
            Snippet::VarOf(e) => {
                let elems = match e {
                    ...
                    other          => (0..n.max(1)).map(|_| other.lit().to_string()).collect::<Vec<_>>(),
                };
            ...
            Snippet::FoldList { elem, fn_name, init } => {
                format!("({op} {fn_name} {} {init})", elem.list(n))
            }

            // tuple
            Snippet::TupleOf(e) => {
                let fields: Vec<_> = (0..n.max(1)).map(|i| format!("(f{i} {})", e.lit())).collect();
            ...
            Snippet::VerifyMerkleProof => {
                ...
                format!("({op} {} {} u0 u{} (list {}))", buf(32), buf(32), n.max(1)+1, s.join(" "))
            }
