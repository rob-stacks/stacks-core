...
pub fn calibrate_from_plus(groups: &std::collections::HashMap<(String, String), Group>)
    -> Option<f64>
{
    let spec = cost_model::for_function("+")?;
    let group = groups.get(&("+".into(), "uint".into()))
        .or_else(|| groups.get(&("+".into(), "int".into())))?;

    let (sum_instrs, sum_cost): (f64, f64) = group.sizes.iter()
        .zip(group.instrs.iter())
        .map(|(&n, &i)| (i as f64, (spec.eval)(n) as f64))
        .filter(|(_, c)| *c > 0.0)
        .fold((0.0, 0.0), |(si, sc), (i, c)| (si + i, sc + c));

    if sum_cost == 0.0 { return None; }
...
pub enum Complexity {
    Constant, Logarithmic, SquareRoot, Linear, NLogN, Quadratic, PowerLaw { k: f64 },
}
...
#[derive(Debug, Clone)]
pub struct Fit { pub complexity: Complexity, pub a: f64, pub b: f64, pub aicc: f64, pub r2: f64 }

fn linreg(x: &[f64], y: &[f64]) -> (f64, f64) {
    let n = x.len() as f64;
    let sx: f64 = x.iter().sum(); let sy: f64 = y.iter().sum();
    let sxx: f64 = x.iter().map(|v| v*v).sum();
    let sxy: f64 = x.iter().zip(y).map(|(xi,yi)| xi*yi).sum();
    let d = n*sxx - sx*sx;
    if d.abs() < f64::EPSILON { return (0.0, sy/n); }
    let a = (n*sxy - sx*sy)/d; ((a), (sy - a*sx)/n)
}

fn aicc_score(n: usize, k: usize, rss: f64) -> f64 {
    let (n, k) = (n as f64, k as f64);
    let corr = if n-k-1.0 > 0.0 { 2.0*k*(k+1.0)/(n-k-1.0) } else { f64::INFINITY };
    n * (rss/n).max(f64::EPSILON).ln() + 2.0*k + corr
}

fn rss_r2(xf: &[f64], y: &[f64], a: f64, b: f64) -> (f64, f64) {
    let rss: f64 = xf.iter().zip(y).map(|(x,yi)| (yi-(a*x+b)).powi(2)).sum();
...
fn fit_one(c: Complexity, x: &[f64], y: &[f64]) -> Option<Fit> {
    ...
    Some(Fit { complexity: c, a, b, aicc: aicc_score(x.len(), k, rss), r2 })
}
...
pub fn select_model(sizes: &[u64], counts: &[u64]) -> Vec<Fit> {
    ...
    .iter().filter_map(|&c| fit_one(c, &x, &y)).collect();
...
fn metric_block(label: &str, sizes: &[u64], counts: &[u64], n_unit: &str) {
    ...
    if w.complexity != Complexity::Constant {
        println!("  │  fit: {}  R²={:.4}  ≈ {:.3}×{} + {:.0}", w.complexity, w.r2, w.a, w.complexity, w.b);
    } else {
...
pub fn print_report(g: &Group, instrs_per_cost_unit: Option<f64>) {
    ...
    if spec.is_some() && instrs_per_cost_unit.is_some() {
        ...
        for (&n, &instrs) in g.sizes.iter().zip(g.instrs.iter()) {
            ...
            println!("  {:>10}  {:>14}  {:>12}  {:>11}  {:>8}",
                n, instrs, current, suggested, fmt_delta(delta));
        }
    }
