...
macro_rules! ctr {
    ($src:expr, $fn:expr, $ustx:expr) => {
        Case {
            ...
            n_unit: "ignored",
            sizes: &[1],
            execution: Execution::Contract { source: $src, fn_name: $fn, initial_ustx: $ustx },
        }
    };
...
fn src_ft_balance(_: u64) -> String {
    "(define-fungible-token tok)\n(define-public (bench) (ok (ft-get-balance tok tx-sender)))".into()
}
...
fn src_nft_transfer(_: u64) -> String {
    "(define-non-fungible-token tok uint)\n\
     ...
     (define-public (bench) (let ((id (+ (var-get ctr) u1))) (var-set ctr id) \
       (try! (nft-mint? tok id tx-sender)) (nft-transfer? tok id tx-sender dst)))".into()
}
fn src_nft_burn(_: u64) -> String {
    "(define-non-fungible-token tok uint)\n\
     (define-data-var ctr uint u0)\n\
     (define-public (bench) (let ((id (+ (var-get ctr) u1))) (var-set ctr id) \
       (try! (nft-mint? tok id tx-sender)) (nft-burn? tok id tx-sender)))".into()
}
fn src_stx_balance(_: u64) -> String { "(define-public (bench) (ok (stx-get-balance tx-sender)))".into() }
fn src_stx_transfer(_: u64) -> String {
    "(define-constant dst 'SPAXYA5XS51713FDTQ8H94EJ4V579CXMTRNBZKSF)\n\
     (define-public (bench) (stx-transfer? u1 tx-sender dst))".into()
}
fn src_stx_burn(_: u64) -> String { "(define-public (bench) (stx-burn? u1 tx-sender))".into() }
fn src_stx_account(_: u64) -> String { "(define-public (bench) (ok (stx-account tx-sender)))".into() }
fn src_stx_transfer_memo(_: u64) -> String {
    "(define-constant dst 'SPAXYA5XS51713FDTQ8H94EJ4V579CXMTRNBZKSF)\n\
     (define-public (bench) (stx-transfer-memo? u1 tx-sender dst 0x00))".into()
}
...
pub static ALL_SUITES: &[Suite] = &[
    suite!("+",   &[ snip!(Snippet::VarOf(ElemType::Uint)),      snip!(Snippet::VarOf(ElemType::Int))      ]),
    suite!("-",   &[ snip!(Snippet::VarLeadUint),  snip!(Snippet::VarLeadInt)  ]),
    suite!("*",   &[ snip!(Snippet::VarOf(ElemType::Uint)),      snip!(Snippet::VarOf(ElemType::Int))      ]),
    suite!("/",   &[ snip!(Snippet::VarLeadUint),  snip!(Snippet::VarLeadInt)  ]),
    suite!("mod", &[ snip!(Snippet::Fixed("uint", "u100 u7")), snip!(Snippet::Fixed("int", "100 7")) ]),
    suite!("pow", &[ snip!(Snippet::Fixed("uint", "u2 u10")),  snip!(Snippet::Fixed("int", "2 10"))  ]),
    ...
    suite!("xor",    &[ snip!(Snippet::XorPair), snip!(Snippet::XorPairInt)   ]),
    suite!(">=", &[ snip!(Snippet::Fixed("uint", "u10 u5")), snip!(Snippet::Fixed("int", "10 5")) ]),
    suite!("<=", &[ snip!(Snippet::Fixed("uint", "u5 u10")), snip!(Snippet::Fixed("int", "5 10")) ]),
    suite!("<",  &[ snip!(Snippet::Fixed("uint", "u5 u10")),  snip!(Snippet::Fixed("int", "5 10"))  ]),
    suite!(">",  &[ snip!(Snippet::Fixed("uint", "u10 u5")),  snip!(Snippet::Fixed("int", "10 5"))  ]),
    suite!("is-eq", &[
    ...
    suite!("not",    &[ snip!(Snippet::Fixed("true", "true")), snip!(Snippet::Fixed("false", "false")) ]),
    suite!("if",     &[ snip!(Snippet::Fixed("true-branch", "true u1 u2")), snip!(Snippet::Fixed("false-branch", "false u1 u2")) ]),
    ...
    suite!("map", &[
        snip!(Snippet::MapList { elem: ElemType::Bool,    fn_name: "not"     }),
        snip!(Snippet::MapList { elem: ElemType::Uint,    fn_name: "to-uint" }),
        snip!(Snippet::MapList { elem: ElemType::OptUint, fn_name: "is-some" }),
    ]),
    suite!("filter", &[
        snip!(Snippet::FilterList { elem: ElemType::Bool,    fn_name: "not"     }),
        snip!(Snippet::FilterList { elem: ElemType::OptUint, fn_name: "is-some" }),
    ]),
    suite!("fold", &[
        snip!(Snippet::FoldList { elem: ElemType::Uint, fn_name: "+",  init: "u0"    }),
        snip!(Snippet::FoldList { elem: ElemType::Bool, fn_name: "or", init: "false" }),
    ]),
    ...
    suite!("sha256",     &[ snip!(Snippet::ApplyBuf),    snip!(Snippet::ApplyStr)    ]),
    suite!("sha512",     &[ snip!(Snippet::ApplyBuf)     ]),
    suite!("sha512/256", &[ snip!(Snippet::ApplyBuf) ]),
    suite!("keccak256",  &[ snip!(Snippet::ApplyBuf)  ]),
    suite!("secp256k1-recover?", &[ snip!(Snippet::Fixed("fixed-inputs", "\
    ...
    suite!("secp256k1-verify", &[ snip!(Snippet::Fixed("fixed-inputs", "\
         0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa \
         0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb \
         0xcccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc")) ]),
    suite!("secp256r1-verify", &[ snip!(Snippet::Fixed("fixed-inputs", "\
         0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa \
         0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb \
         0xcccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc")) ]),
    suite!("print", &[ snip!(Snippet::Fixed("uint", "u42")), snip!(Snippet::ApplyStr), snip!(Snippet::ApplyBuf) ]),
    suite!("some",             &[ snip!(Snippet::Fixed("uint", "u42"))  ]),
    suite!("ok",               &[ snip!(Snippet::Fixed("uint", "u42"))    ]),
    suite!("err",              &[ snip!(Snippet::Fixed("uint", "u42"))   ]),
    suite!("default-to",       &[ snip!(Snippet::Fixed("some", "u0 (some u1)")), snip!(Snippet::Fixed("none", "u0 none")) ]),
    suite!("unwrap!",          &[ snip!(Snippet::Fixed("some", "(some u1) u0"))        ]),
    suite!("unwrap-err!",      &[ snip!(Snippet::Fixed("err", "(err u1) u0"))     ]),
    suite!("unwrap-panic",     &[ snip!(Snippet::Fixed("some", "(some u1)"))      ]),
    suite!("unwrap-err-panic", &[ snip!(Snippet::Fixed("err", "(err u1)"))   ]),
    suite!("try!", &[ snip!(Snippet::Fixed("ok", "(ok u1)")), snip!(Snippet::Fixed("some", "(some u1)")) ]),
    suite!("is-ok",   &[ snip!(Snippet::Fixed("ok", "(ok u1)")),   snip!(Snippet::Fixed("err", "(err u1)"))   ]),
    suite!("is-none", &[ snip!(Snippet::Fixed("none", "none")),     snip!(Snippet::Fixed("some", "(some u1)")) ]),
    suite!("is-err",  &[ snip!(Snippet::Fixed("err", "(err u1)")),  snip!(Snippet::Fixed("ok", "(ok u1)"))   ]),
    suite!("is-some", &[ snip!(Snippet::Fixed("some", "(some u1)")),snip!(Snippet::Fixed("none", "none"))      ]),
    ...
    suite!("is-standard",       &[ snip!(Snippet::Fixed("standard-principal", "'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR")) ]),
    suite!("principal-destruct?",&[ snip!(Snippet::Fixed("standard-principal", "'SP2C2YFP12AJZB4MABJBAJ55XECVS7E4PMMZ89YZR")) ]),
    suite!("principal-construct?", &[
        snip!(Snippet::Fixed("standard", "0x16 0xfa6bf38ed557fe417333710d6033e9419391a320")),
        snip!(Snippet::Fixed("contract", "(principal-construct? 0x16 0xfa6bf38ed557fe417333710d6033e9419391a320 \"my-contract\")")),
    ]),
    suite!("string-to-int?",  &[ snip!(Snippet::StringToIntAscii),  snip!(Snippet::StringToIntUtf8)  ]),
    suite!("string-to-uint?", &[ snip!(Snippet::StringToIntAscii), snip!(Snippet::StringToIntUtf8) ]),
    suite!("int-to-ascii", &[ snip!(Snippet::Fixed("uint", "u123456789")), snip!(Snippet::Fixed("int", "-123456789")) ]),
    suite!("int-to-utf8",  &[ snip!(Snippet::Fixed("uint", "u123456789")),  snip!(Snippet::Fixed("int", "-123456789"))  ]),
    suite!("bit-and",        &[ snip!(Snippet::VarOf(ElemType::Uint)),      snip!(Snippet::VarOf(ElemType::Int))      ]),
    suite!("bit-or",         &[ snip!(Snippet::VarOf(ElemType::Uint)),       snip!(Snippet::VarOf(ElemType::Int))       ]),
    suite!("bit-not",        &[ snip!(Snippet::Fixed("uint", "u42")),          snip!(Snippet::Fixed("int", "42"))          ]),
    suite!("bit-shift-left", &[ snip!(Snippet::Fixed("uint", "u1 u4")), snip!(Snippet::Fixed("int", "1 u4")) ]),
    suite!("bit-shift-right",&[ snip!(Snippet::Fixed("uint", "u256 u4")), snip!(Snippet::Fixed("int", "256 u4")) ]),
    suite!("bit-xor",        &[ snip!(Snippet::VarOf(ElemType::Uint)),      snip!(Snippet::VarOf(ElemType::Int))      ]),
    suite!("slice?", &[
    ...
    suite!("to-consensus-buff?",   &[ snip!(Snippet::Fixed("uint", "u42")), snip!(Snippet::ToConsensusBuf)  ]),
    suite!("from-consensus-buff?", &[ snip!(Snippet::Fixed("int", "int 0x000000000000000000000000000000000000")), snip!(Snippet::FromConsensusBuf) ]),
    suite!("to-ascii?", &[ snip!(Snippet::ApplyUtf8) ]),
    // Clarity 6
    suite!("verify-merkle-proof",   &[ snip!(Snippet::VerifyMerkleProof)  ]),
    suite!("get-bitcoin-tx-output?",&[ snip!(Snippet::GetBitcoinTxOutput) ]),
    suite!("ed25519-verify",        &[ snip!(Snippet::Ed25519Verify)      ]),
    suite!("secp256k1-decompress?", &[ snip!(Snippet::Fixed("compressed-key", "0x02abababababababababababababababababababababababababababababababababab")) ]),
];
...
pub static STATEFUL_SUITES: &[Suite] = &[
    suite!("var-get",    &[ ctr!("buffer", "value_bytes", BUFS, src_var_get,    "bench", 0) ]),
    suite!("var-set",    &[ ctr!("buffer", "value_bytes", BUFS, src_var_set,    "bench", 0) ]),
    suite!("map-get?",   &[ ctr!("buffer", "value_bytes", BUFS, src_map_get,    "bench", 0) ]),
    suite!("map-set",    &[ ctr!("buffer", "value_bytes", BUFS, src_map_set,    "bench", 0) ]),
    suite!("map-insert", &[ ctr!("buffer", "value_bytes", BUFS, src_map_insert, "bench", 0) ]),
    suite!("map-delete", &[ ctr!("buffer", "value_bytes", BUFS, src_map_delete, "bench", 0) ]),
    ...
    suite!("stx-transfer?",     &[ ctr!(src_stx_transfer,      "bench", 1_000_000_000_000) ]),
    suite!("stx-burn?",         &[ ctr!(src_stx_burn,          "bench", 1_000_000_000_000) ]),
    suite!("stx-account",       &[ ctr!(src_stx_account,       "bench", 0)                 ]),
    suite!("stx-transfer-memo?",&[ ctr!(src_stx_transfer_memo, "bench", 1_000_000_000_000) ]),
    suite!("get-block-info?",        &[ ctr!(src_block_info,        "bench", 0) ]),
    suite!("get-stacks-block-info?", &[ ctr!(src_stacks_block_info, "bench", 0) ]),
    suite!("get-tenure-info?",       &[ ctr!(src_tenure_info,       "bench", 0) ]),
    suite!("get-burn-block-info?",   &[ ctr!(src_burn_block_info,   "bench", 0) ]),
    suite!("contract-call?", &[ ctr!(
        |_| "(define-public (target) (ok true))\n             (define-public (bench) (contract-call? .bench target))".to_string(),
        "bench", 0) ]),
...
pub fn find(function: &str, variant: &str) -> Option<(&'static Suite, &'static Case)> {
    all_suites().find_map(|s| {
        if s.function != function { return None; }
        s.cases.iter().find(|c| c.variant == variant).map(|c| (s, c))
    })
}
